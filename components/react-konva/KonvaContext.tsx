...
import {
  ...
  Artboard,
  Transform,
} from "./types"; // Assume types are in a separate file
import Konva from "konva";

interface KonvaContextType {
  structure: KonvaStructure;
  addLayer: (layer: Layer) => void;
  addGroup: (group: Group, parentId?: string | null) => void;
  addObject: <T = Konva.NodeConfig>(
    obj: KonvaObject<T>,
    artboardId?: string | null
  ) => void;
  addArtboard: (artboard: Artboard) => Artboard;
...
> = ({ initialStructure, children }) => {
  ...
  const addGroup = (group: Group, parentId?: string | null) => {
    saveHistory();
  ...
  const addObject: KonvaContextType["addObject"] = (obj, artboardId) => {
    setHistory((prev) => [structure, ...prev]);
    setStructure((prevStructure) => {
      const updatedArtboards = prevStructure.artboards.map((artboard) => {
        if (artboard.id === artboardId) {
          return { ...artboard, objects: [...artboard.objects, obj] };
        }
        return artboard as any;
      });
      return { ...prevStructure, artboards: updatedArtboards };
    });
  };
  ...
  const addArtboard = (artboard: Artboard) => {
    ...
    setStructure((prev) => ({
      ...prev,
      artboards: [...prev.artboards, artboard],
    }));
    return artboard;
  };
